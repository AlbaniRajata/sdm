...
class _CustomFilterState<T> extends State<CustomFilter<T>> {
  ...
  List<T> _getSortOptionPair(String label) {
    ...
    } else if (widget.sortOptions[0] is DosenSortOption) {
      switch (label) {
        case 'Abjad':
          return [DosenSortOption.abjadAZ, DosenSortOption.abjadZA] as List<T>;
        case 'Poin':
          return [DosenSortOption.poinTerbanyak, DosenSortOption.poinTersedikit] as List<T>;
      }
    } else if (widget.sortOptions[0] is PenggunaSortOption) {
      switch (label) {
        case 'Abjad':
          return [PenggunaSortOption.abjadAZ, PenggunaSortOption.abjadZA] as List<T>;
      }
    }
    return [];
  }
  ...
  String _getOptionLabel(T option) {
    if (option is SortOption || 
        option is KegiatanSortOption || 
        option is DosenSortOption || 
        option is PenggunaSortOption) {
      ...
      if (option.toString().contains('nonJTI')) return 'Kegitan Non-JTI';
      if (option.toString().contains('admin')) return 'Admin';
      if (option.toString().contains('dosen')) return 'Dosen';
      if (option.toString().contains('pimpinan')) return 'Pimpinan';
    }
    return '';
  }
  ...
  Widget _buildChip(String label, {List<T>? sortOptionPair, T? singleOption}) {
    ...
    return FilterChip(
      ...
      showCheckmark: false, // Menghilangkan icon centang
      selected: isSelected,
